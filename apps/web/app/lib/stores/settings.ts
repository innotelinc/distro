...
const getInitialProviderSettings = (): ProviderSetting => {
  ...
  PROVIDER_LIST.forEach((provider) => {
    initialSettings[provider.name] = {
      ...provider,
      settings: {
        // Local providers should be disabled by default, EXCEPT OpenAILike:
        // in Distro, OpenAILike is the OmniRoute gateway provider and is
        // enabled out of the box so it shows up as the default provider.
        enabled: provider.name === 'OpenAILike' || !LOCAL_PROVIDERS.includes(provider.name),
      },
    };
  });
