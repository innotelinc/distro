...
export class LLMManager {
  ...
  getDefaultProvider(): BaseProvider {
    // Distro: provider registration iterates a module namespace whose exports
    // enumerate alphabetically, so "first registered" is arbitrary. Make the
    // OmniRoute gateway provider (OpenAILike) the deterministic default; if it
    // is ever removed, fall back to the first registered provider.
    const gateway = this._providers.get('OpenAILike');
    const firstProvider = this._providers.values().next().value;
    const provider = gateway ?? firstProvider;
